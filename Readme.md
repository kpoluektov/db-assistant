...
**Что такое DB Assistant.** Веб-приложение, в котором с базой данных можно общаться на естественном языке. Пользователь задаёт вопрос на русском («сколько пассажиров летело из Москвы в июле?») — приложение само пишет SQL, выполняет его и возвращает ответ. Это инструмент для аналитиков, разработчиков и DBA, который даёт им возможность работать с БД без прямого доступа и без знания SQL.

**Как это работает.** При старте приложение само читает структуру выбранной схемы — таблицы, колонки, связи, индексы — и подкладывает её в системный промт. Благодаря этому модель «знает» базу и пишет корректный SQL без уточняющих вопросов к пользователю. Внутри работает связка из трёх агентов: главный агент-маршрутизатор принимает вопрос и передаёт его либо агенту по метаданным (структура, индексы, статистика, оптимизация), либо агенту по данным (генерация и выполнение SQL). Все обращения к БД идут через MCP-сервер в режиме read-only — данные нельзя изменить или удалить. В интерфейсе слева — диалог с ассистентом, справа — SQL-консоль, в которой в реальном времени видно, какие запросы агент сформировал и выполнил.

Под капотом — [OpenAI Agents SDK](https://github.com/openai/openai-agents-python) поверх Yandex AI Studio (паттерн Handoff между агентами) и три микросервиса в Docker Compose.

![Интерфейс DB Assistant](docs/screenshot01.png)

*Слева — диалог с ассистентом на русском, справа — SQL-консоль с пресетами и результатом последнего запроса. Блок «Рассуждения» раскрывает шаги агента: какие инструменты MCP были вызваны и в каком порядке.*

## Что это даёт бизнесу

- **Self-service для нетехнических пользователей.** Аналитики, продакты и менеджеры получают ответы из БД на естественном языке, без задачи разработчику и без знания SQL.
- **Быстрый онбординг в незнакомую схему.** Новый сотрудник или подрядчик спрашивает «как у вас устроены заказы?» — агент сам находит нужные таблицы, связи и описывает их.
- **Аудит структуры и оптимизация без DBA.** Проверить наличие индексов, актуальность статистики, дерево внешних ключей или получить рекомендацию по ускорению запроса можно прямо из чата.
- **Безопасность по умолчанию.** Все обращения к БД — read-only, каждый сгенерированный SQL виден в консоли. В модель уходит только структура схемы, сами данные остаются в вашем контуре.
...
Несколько типовых сценариев. Полный пошаговый сценарий показа на демо-схеме `demo` (авиарейсы) — от простых выборок до многотабличных JOIN и оптимизации — лежит в [`examples/README.md`](examples/README.md).
...
Агент проверит индексы на `users.id` и `orders.user_id`, посмотрит статистику таблиц и предложит конкретные изменения.
...
Разберём на конкретном примере, что происходит между моментом, когда пользователь нажимает Enter, и моментом, когда в чате появляется ответ. Вопрос: **«Сколько пассажиров летело из Москвы в июле 2026?»**

**1. Старт приложения (один раз).** Контейнер `agent` через MCP-сервер считывает структуру указанной схемы БД — список таблиц, их колонки, индексы, внешние ключи — и записывает всё в файл `AGENT.md`. При обработке каждого следующего вопроса этот файл подкладывается в системный промт агентов: модель «знает» структуру БД и пишет корректный SQL без уточняющих вопросов к пользователю. **В LLM уходит только структура схемы, сами строки таблиц никогда не отправляются в модель** — за исключением результатов конкретных SQL-запросов, которые агент выполнил, чтобы ответить на ваш вопрос.

**2. Маршрутизация.** Главный агент `AssistantAgent` видит вопрос и решает, кому его передать: вопросы про структуру, индексы, оптимизацию идут в `MetadataAgent`, вопросы про сами данные — в `DataAgent`. Передача оформлена паттерном handoff из OpenAI Agents SDK. В нашем примере вопрос про данные → передаётся `DataAgent`.

**3. Генерация SQL.** `DataAgent` смотрит на схему в `AGENT.md`, понимает, что нужно соединить `boardings`, `flights` и `airports`, и формирует SQL:
...
**4. Вызов инструмента через MCP.** Чтобы выполнить SQL, агент не идёт в БД напрямую — он вызывает MCP-инструмент `run_wide_sql(sql=...)`. Это и есть **MCP** (Model Context Protocol — открытый стандарт от Anthropic для подключения инструментов к LLM-агентам): MCP-сервер выставляет каталог из 7 инструментов (`get_metadata`, `get_indexes`, `get_relationships`, `run_wide_sql` и т.д.), а агент в ходе рассуждения выбирает нужный и вызывает его — так же, как мог бы звать обычную функцию. Вынос инструментов в отдельный сервис даёт переиспользуемость: тот же MCP-сервер можно подключить к Claude Desktop, Cursor или любому другому MCP-совместимому клиенту, не меняя ни строчки в самих инструментах.

**5. Исполнение.** MCP-сервер передаёт SQL в `metadata_server`, тот открывает в БД read-only транзакцию, выполняет запрос и возвращает результат. Read-only — это не настройка приложения, которую агент мог бы обойти: транзакция стартует с уровнем `READ ONLY`, любой `INSERT`/`UPDATE`/`DELETE` в её рамках упадёт на уровне СУБД.

**6. Ответ.** `DataAgent` получает строку с числом, формулирует ответ на естественном языке («в июле 2026 из Москвы было сделано N посадок») и возвращает его пользователю. Параллельно UI рисует выполненный SQL и таблицу строк в правой SQL-консоли — пользователь видит, **как именно** агент пришёл к ответу, и при необходимости может поправить SQL руками.
...
Три микросервиса запускаются через Docker Compose в изолированной сети `mcp_network`. Веб-интерфейс отдаёт сам контейнер `agent` (FastAPI + Socket.IO), SQL-консоль умеет ручной ввод запросов и преднастроенные пресеты — см. параметр `YANDEX__SQL_PRESETS` в разделе «Быстрый старт».
...
> **Нет своей БД для демо?** Используйте готовую схему из каталога `examples/` — PostgreSQL, схема `demo` про авиарейсы (10 аэропортов, ~38 000 посадок за июль 2026). К ней уже подобраны вопросы для демонстрации разных сценариев работы агента. Инструкция по загрузке и сценарий показа — в [`examples/README.md`](examples/README.md).
...
### Заметка: `SESSION_MAX_HISTORY` vs `MAX_TURNS`

Две настройки агента, которые часто путают, — они управляют разными вещами:
